 ...
 * @author Sarvesh Kulkarni <sarvesh.mak@gmail.com>
 * @version 1.0.0
 ...
 * @class HoverFilter
 * @augments PIXI.Filter
 * @author Sarvesh Kulkarni <liam@wethecollective.com>
 * @version 1.0.0
...
class Navigation {
  ...
  makeNavItem(title) {
    ...
    ctx.fillStyle = "rgba(239, 220, 220,1)";
  ...
  setupWebGLContext() {
    ...
    this.app.stage.x = window.innerWidth * 0.5;
    this.app.stage.y = window.innerHeight * 0.5;
    ...
    this.navItems.forEach(item => {
      item.sprite.x = this.navWidth * -0.5 + ipos; // Calculate the position of the nav element to the nav width
  ...
  focusNavItemByIndex(index) {
    ...
    let mousepos = [
      window.innerWidth * 0.1 + window.innerWidth * 0.8 * c,
  ...
  setupBackground() {
    ...
    this.background.drawRect(
      -this.maskpadding,
      -this.maskpadding,
      window.innerWidth + this.maskpadding,
      window.innerHeight + this.maskpadding
    );
    ...
    mask.beginFill(this.backgroundColour, 0.5);
    mask.position.x = window.innerWidth * -0.5;
    mask.position.y = window.innerHeight * -0.5;
    mask.drawRect(
      -this.maskpadding,
      -this.maskpadding,
      window.innerWidth + this.maskpadding,
  ...
  fixMousePos(mousepos_px) {
    ...
    if (window.innerHeight > window.innerWidth) {
      mousepos[0] =
        (mousepos_px[0] - window.innerWidth / 2) / window.innerWidth;
      mousepos[1] =
        ((mousepos_px[1] - window.innerHeight / 2) / window.innerHeight) *
        -1 *
        ratio;
    } else {
      mousepos[0] =
        (mousepos_px[0] - window.innerWidth / 2) / window.innerWidth / ratio;
      mousepos[1] =
        ((mousepos_px[1] - window.innerHeight / 2) / window.innerHeight) * -1;
    }
    return mousepos;
  }
  ...
  unfixMousePos(mousepos) {
    ...
    if (window.innerHeight > window.innerWidth) {
      ...
      mousepos_px[1] =
        (mousepos[1] * window.innerHeight) / -1 / ratio +
        window.innerHeight / 2;
    } else {
      mousepos_px[0] =
        mousepos[0] * window.innerWidth * ratio + window.innerWidth / 2;
      mousepos_px[1] =
        (mousepos[1] * window.innerHeight) / -1 + window.innerHeight / 2;
    }
  ...
  onOpen() {
    ...
    this.mousepos = [3000, window.innerHeight * 0.5];
  }
  /**
  ...
  animate(delta) {
    ...
    const diff = [
      this.targetMousePos[0] - pxMousepos[0],
      this.targetMousePos[1] - pxMousepos[1]
    ];
  ...
  get backgroundColour() {
    return this._backgroundColour || 0x1c1c21;
  }
  ...
  set mousepos(value) {
    ...
    this.container.position.x =
      -(this.navWidth * 0.5) + (1 - p) * this.navWidth;
...
document.addEventListener("keyup", e => {
  if (e.target.className.indexOf("nav-toggle") && e.keyCode === 13) {
...
tl.staggerTo(
  firstWord,
  1,
  { color: "rgb(31, 122, 145)", rotationX: 360, ease: Expo.easeOut },
  0.03,
  "#start"
);
tl.staggerTo(
